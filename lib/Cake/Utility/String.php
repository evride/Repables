 ...
 */
	...
	public static function replaceURLs($str, $postProcess = array()){
	//
		//			([\\s(<.,;:!?])                                        # 1: Leading whitespace, or punctuation
		$pattern = '~(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'".,<>?«»“”‘’]))~';
		$matchedUrls = array();
		while( preg_match($pattern, $str, $matches, PREG_OFFSET_CAPTURE)){
			$hash = md5($matches[0][0]);
			$matchedUrls[$hash] = $matches[0][0];
			$str = substr_replace($str, $hash, $matches[0][1], strlen($matches[0][0]));
		}
		foreach($postProcess as $method){
			$str = call_user_func($method, $str);
		}
		foreach($matchedUrls as $hash=>$url){
			$str = str_replace($hash, '<a rel="nofollow" href="' . $url . '">' . htmlentities($url) . '</a>', $str);
		}
		return $str;
	}
